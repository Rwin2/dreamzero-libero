...
### Zero-Shot Experiments (Pretrained DROID Model)

Before fine-tuning, I tested the pretrained DROID model (which natively outputs delta joint positions) on LIBERO with different controller configurations to understand the action space gap:

1. **OSC_POSE controller**: Model outputs joint deltas, controller expects Cartesian deltas. Complete semantic mismatch -- robot moves erratically. 0% success.
2. **JOINT_POSITION controller, raw output**: Correct action type, but robosuite's JOINT_POSITION controller internally scales input by 0.05. Model outputs ~0.5 rad deltas, controller receives 0.025 rad. Robot barely moves. 0% success.
3. **JOINT_POSITION controller, rescaled (/ 0.05)**: Correct action type and magnitude. Robot moves at reasonable speed, but actions are not task-directed (model was trained on DROID scenes, not LIBERO). 0% success.

These experiments confirmed that even with the correct controller, zero-shot transfer fails due to the visual domain gap. Fine-tuning is necessary.

## Training

### Loss Function

DreamZero jointly trains video prediction and action prediction through flow matching:

- **Dynamics loss**: MSE between predicted and target noise in the video latent space. Measures world model quality.
- **Action loss**: MSE between predicted and target noise in the action space. Measures policy quality.

Both use flow matching (continuous-time diffusion). Total loss = dynamics_loss + action_loss.
...
For reference, DreamZero-DROID was trained with 80K steps on **32 H100s over ~3 days**. This LoRA fine-tuning on 500 LIBERO demos (10 tasks x 50 demos) is **0.25%** of that compute.

### Training Results (200 steps, ~30 min)
...
Action loss decreased 64% (0.191 -> 0.068). Dynamics loss roughly stable -- visual domain shift partially absorbed without catastrophic forgetting.

## Evaluation Results

### Closed-Loop: LIBERO-Spatial (10 tasks, 1 trial each)

| Metric | Zero-Shot (DROID) | Fine-Tuned (200 steps) |
|---|---|---|
| Success rate | 0/10 (0%) | 0/10 (0%) |
| Eval time | ~13 min | ~13 min |

### Offline Action Comparison

To get a continuous metric beyond binary success, I compared model-predicted actions against expert actions from the training data (first 3 episodes per task, 8 steps each, 240 samples total).
...
The model does not condition on visual input after 200 steps. It outputs the same action for every scene: approximately [-0.47, -0.48, -0.16] in (dx, dy, dz), which is opposite to the expert mean [+0.42, +0.15, +0.11]. The DROID pretrained prior dominates.

### Action Space Limitation

LIBERO only stores delta EE actions (OSC_POSE). There is no joint position data. The fine-tuned model learned a different action space than DROID's native joint space. With inverse kinematics -- computing J(q)^{-1} * dx using the Franka Panda's kinematic model and the recorded joint configurations -- one could recover the corresponding joint position actions and train in the model's native space. This would better leverage the pretrained weights. The Franka URDF is publicly available and standard robotics libraries (pinocchio, roboticstoolbox-python) provide the IK solver. This is analogous to classic Puma arm IK derivations.

### Scaling Up

| Steps | Epochs | Time (2xH100) | Expected Outcome |
|---|---|---|---|
...
Distribution shift during closed-loop rollouts could be addressed by collecting failure trajectories with the current model in LIBERO and re-training on the aggregated dataset.

## Quick Start

### Prerequisites

- 2x H100 GPUs (80GB each)
...
conda activate dreamzero
cd libero_work
python offline_comparison.py  # requires inference server running
```
